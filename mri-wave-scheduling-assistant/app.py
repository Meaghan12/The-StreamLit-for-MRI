...
if 'queue_df' not in st.session_state:
    default_csv = os.path.join(SCRIPT_DIR, "sample_patient_queue.csv")
    if os.path.exists(default_csv):
        st.session_state.queue_df = pd.read_csv(default_csv)
    else:
...
# --- TABS ---
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Dashboard", "Patient Queue", "Lego-Block Ranking", "Scenario Comparison", "Export"])
...
with tab3:
    ...
    if df.empty:
        ...
        def calculate_score(row):
            ...
            try:
                if int(row['estimated_duration']) <= gap_minutes:
                    score += 40
        ...
        st.dataframe(ranked_df[['Rank','patient_id','protocol','scanner_eligibility','estimated_duration','Score','Why selected']], use_container_width=True, hide_index=True)
...
with tab4:
    ...
    bar_path = os.path.join(SCRIPT_DIR, "assets", "utilization_bar.png")
    with col1:
        if os.path.exists(box_path):
            st.image(box_path, caption="Staytime Comparison (Winsorized)", use_container_width=True)
        else:
            st.info("Staytime chart not found in assets/")
    with col2:
